...
FRepObj2D::FRepObj2D(int resX, int resY): m_resX(resX), m_resY(resY) { }

float FRepObj2D::triangle(glm::vec2 pos, glm::vec2 cent, float a, float b, float c, const float scale)
{
...
float FRepObj2D::triangle2(glm::vec2 pos, glm::vec2 cent, const float a, const float b, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
...
float FRepObj2D::circle(glm::vec2 pos, glm::vec2 cent, float R, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
...
float FRepObj2D::blobby2D(glm::vec2 pos, glm::vec2 cent, float R, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    R = convertToUV(scaleToNewRange(R));
...
float FRepObj2D::ellipticCylZ2D(glm::vec2 pos, glm::vec2 cent, float R, float a, float b, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    R = convertToUV(scaleToNewRange(R));
...
float FRepObj2D::ellipsoid2D(glm::vec2 pos, glm::vec2 cent, float R, float a, float b, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    R = convertToUV(scaleToNewRange(R));
...
float FRepObj2D::torusY2D(glm::vec2 pos, glm::vec2 cent, float R, float rev, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    R = convertToUV(scaleToNewRange(R));
...
float FRepObj2D::torusZ2D(glm::vec2 pos, glm::vec2 cent, float R, float rev, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    R = convertToUV(scaleToNewRange(R));
...
float FRepObj2D::rectangle(glm::vec2 pos, glm::vec2 cent, float w, float h, const float scale)
{
    glm::vec2 uv  = convertToUV(pos);
    glm::vec2 c0  = convertToUV(cent);
...
float FRepObj2D::heart2D(glm::vec2 pos, glm::vec2 cent, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    float shX = scale*(uv.x - c0.x);
...
float FRepObj2D::decocube2D(glm::vec2 pos, glm::vec2 cent, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    float shX = scale*(uv.x - c0.x);
    ...
    return -(std::pow(shX*shX + shY*shY - 2.0f, 2.0f) + 1.0f) *
            ...
            (std::pow(shX*shX - 2.0f, 2.0f) + std::pow(shY*shY-1.0f, 2.0f)) + 50.0f/m_resX;
}

float FRepObj2D::suriken(glm::vec2 pos, glm::vec2 cent, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    glm::vec2 c0 = convertToUV(cent);
    float shX = scale*(uv.x - c0.x);
    ...
    float lb1 = shY + scaleToNewRange(358.4)/m_resY - ( shX*2.0f + scaleToNewRange(153.6)/m_resX);
    float rb1 = shY + scaleToNewRange(358.4)/m_resY - (-shX*2.0f + scaleToNewRange(153.6)/m_resX);
    float lb2 = shY - scaleToNewRange(51.2)/m_resY  - ( shX*2.0f + scaleToNewRange(153.6)/m_resY);
    float rb2 = shY - scaleToNewRange(51.2)/m_resY  - (-shX*2.0f + scaleToNewRange(153.6)/m_resY);
    float lb3 = shX + scaleToNewRange(358.4)/m_resX - ( shY*2.0f + scaleToNewRange(153.6)/m_resY);
    float rb3 = shX + scaleToNewRange(358.4)/m_resX - (-shY*2.0f + scaleToNewRange(153.6)/m_resY);
    float lb4 = shX - scaleToNewRange(51.2)/m_resX  - ( shY*2.0f + scaleToNewRange(153.6)/m_resY);
    float rb4 = shX - scaleToNewRange(51.2)/m_resX  - (-shY*2.0f + scaleToNewRange(153.6)/m_resY);

    float trian1 = intersect_function(intersect_function( lb1,  rb1), -shY+scaleToNewRange(102.4)/m_resY);
    float trian2 = intersect_function(intersect_function(-lb2, -rb2),  shY+scaleToNewRange(102.4)/m_resY);
    float trian3 = intersect_function(intersect_function( lb3,  rb3), -shX+scaleToNewRange(102.4)/m_resX);
    float trian4 = intersect_function(intersect_function(-lb4, -rb4),  shX+scaleToNewRange(102.4)/m_resX);

    return union_function(union_function(union_function(trian1, trian2), trian3), trian4);
}

float FRepObj2D::elf(glm::vec2 pos, glm::vec2 center, const float scale)
{
    glm::vec2 uv = convertToUV(pos);
    ...
    float mouth2   = -(center.y - scaleToNewRange(20)/scale)/m_resY + uv.y;
    ...
    float body2 = intersect_function(intersect_function(body1, (center.y + scaleToNewRange(30)/scale)/m_resY+uv.y),
                                                              -(center.y + scaleToNewRange(40)/scale)/m_resY+uv.y);
    ...
    float belly6 = -(center.y + scaleToNewRange(20)/scale)/m_resY + uv.y;
    ...
    float must11 = intersect_function(uv.y, (center.y + scaleToNewRange(5)/scale)/m_resY - uv.y);
    ...
    float ude2   = uv.y + (center.y + scaleToNewRange(60)/scale)/m_resY;
    float ude3   = (center.y + scaleToNewRange(60)/scale)/m_resY - uv.y;
...
float FRepObj2D::bat(glm::vec2 pos, glm::vec2 center, const float scale)
{
    //glm::vec2 center = glm::vec2(m_resX/2.0f, m_resY/2.0f);
...
float FRepObj2D::trebleClef(glm::vec2 pos, glm::vec2 center, const float scale)
{
    //glm::vec2 center = glm::vec2(m_resX/2.0f, m_resY/2.0f);
...
std::vector<float> FRepObj2D::getFRep2D(std::function<float(glm::vec2)> fun)
{
    ...
    for(int y = 0; y < m_resY; y++)
        for(int x = 0; x < m_resX; x++)
            m_frep.push_back(fun(glm::vec2(x, y)));
...
std::vector<float> FRepObj2D::getFRep2D(glm::vec2 cent, std::function<float(glm::vec2, glm::vec2)> fun)
{
    ...
    for(int y = 0; y < m_resY; y++)
        for(int x = 0; x < m_resX; x++)
            m_frep.push_back(fun(glm::vec2(x, y), cent));

    return m_frep;
}

std::vector<float> FRepObj2D::getFRep2D(glm::vec2 cent, float R, std::function<float(glm::vec2, glm::vec2, float)> fun)
{
    m_frep.clear();
    for(int y = 0; y < m_resY; y++)
        for(int x = 0; x < m_resX; x++)
            m_frep.push_back(fun(glm::vec2(x, y), cent, R));
                                        ...
                                        std::function<float(glm::vec2, glm::vec2, float, float)> fun)
{
    ...
    for(int y = 0; y < m_resY; y++)
        for(int x = 0; x < m_resX; x++)
            m_frep.push_back(fun(glm::vec2(x, y), cent, p1, p2));
                                        ...
                                        std::function<float(glm::vec2, glm::vec2, float, float, float)> fun)
{
    ...
    for(int y = 0; y < m_resY; y++)
        for(int x = 0; x < m_resX; x++)
            m_frep.push_back(fun(glm::vec2(x, y), cent, p1, p2, p3));
                                        ...
                                        std::function<float (float, float, float, float)> fun)
{
    ...
    for(int i = 0; i < m_resX*m_resY; i++)
        m_frep.push_back(fun(f1[i], f2[i], alpha, m));

    return m_frep;
}

std::vector<float> FRepObj2D::getFRep2D(std::vector<float> f1, std::vector<float> f2, float n,
                                        std::function<float (float, float, float)> fun)
{
    m_frep.clear();
    for(int i = 0; i < m_resX*m_resY; i++)
        m_frep.push_back(fun(f1[i], f2[i], n));
...
std::vector<float> FRepObj2D::getRotatedFrep2D(glm::vec2 cent, float w, float h, float angle,
                                               std::function<float (glm::vec2, glm::vec2, float, float)> fun)
{
    m_frep.clear();
    ...
    for(int y = 0; y < m_resY; y++)
        for(int x = 0; x < m_resX; x++)
            m_frep.push_back(fun(getRotatedCoords(glm::vec2(x,y), angle), c0, w, h));
                                               ...
                                               std::function<float (glm::vec2, glm::vec2, float, float, float, float)> fun)
{
    ...
    for(int y = 0; y < m_resY; y++)
        for(int x = 0; x < m_resX; x++)
            m_frep.push_back(fun(getRotatedCoords(glm::vec2(x, y), angle), c0, a, b, c, angle));
